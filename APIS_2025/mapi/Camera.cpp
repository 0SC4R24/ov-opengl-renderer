...
old::Camera::Camera(glm::vec4 pos, glm::vec4 lookAt, double fovy, double aspectRatio) :
...
void old::Camera::step(float timeStep)
{
    float speed = 1.0f;
    float rotSpeed = 60.0f;

    if (old::GLFWKeyManager::keyboardState[GLFW_KEY_D])
    {
        this->position.x += speed * timeStep;
        lookAt.x += speed * timeStep;
    }
    if (old::GLFWKeyManager::keyboardState[GLFW_KEY_A])
    {
        this->position.x -= speed * timeStep;
        lookAt.x -= speed * timeStep;
    }
    if (old::GLFWKeyManager::keyboardState[GLFW_KEY_W])
    {
        this->position.z -= speed * timeStep;
        lookAt.z -= speed * timeStep;
    }
    if (old::GLFWKeyManager::keyboardState[GLFW_KEY_S])
    {
        this->position.z += speed * timeStep;
        lookAt.z += speed * timeStep;
    }
